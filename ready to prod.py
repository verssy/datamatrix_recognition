...
def main():
    ...
    _, image = cap.read()
    bounds = image.shape[:-1] # (height, width)
    ...
    while True:
        ...
        for finder in fps:
            if finder.c1.x > delta and finder.c1.y > delta and finder.c1.x < bounds[1] and finder.c1.y < bounds[0]:
                ...
                x_min = int(max(circle_coords[0] - circle_radius - 25, 0))
                x_max = int(min(circle_coords[0] + circle_radius + 25, bounds[1]))
                y_min = int(max(circle_coords[1] - circle_radius - 25, 0))
                y_max = int(min(circle_coords[1] + circle_radius + 25, bounds[0]))

                temp_frame = image[y_min:y_max, x_min:x_max]
                temp_frame = rotate_bound(image, angel)
                image_data = pytesseract.image_to_string(temp_frame, config=custom_config)
                ...
                ## TODO: Change to cv2.rectangle()
                #cv2.line(image, (finder.c1.x, finder.c1.y), (finder.c2.x, finder.c2.y), (0, 0, 255), 3)
                #cv2.line(image, (finder.c1.x, finder.c1.y), (finder.c3.x, finder.c3.y), (0, 0, 255), 3)
        ...
        if new_date_found: cv2.putText(image, f'{str_m}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        elif len(curr_date) > 0: cv2.putText(image, f'{str_m}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
