...
min_area = 1000
delta_xy = 10
bool_save_images = False
bool_video = True
bool_fast_decode = False

if bool_save_images:
    imgs_exists = os.path.exists("imgs")
    if not imgs_exists:
        os.makedirs("imgs")
...
if bool_video: cap = cv2.VideoCapture('./vid_2.mp4')
...
while True:
    ...
    # text recognition
    recognized = text_reader.readtext(frame, paragraph=True)
    ...
    if bool_save_images:
        for con in contours:
            ...
            if not (w / h > max_sides_ratio or h / w > max_sides_ratio):
                ...
                out_filename = str(files_prefix + str(files_count)) + ".jpeg"
                cv2.imwrite(out_filename, tempimage)
    ...
    for con in contours:
        ...
        if not (w / h > max_sides_ratio or h / w > max_sides_ratio):
            ...
            else: data = pdmtx.decode(tempimage)
            text = ""
            if len(data) > 0:
    ...
    if bool_video: images.append(frame)
